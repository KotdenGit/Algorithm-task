...
//6. здача на цепочку ошибок
// catch() можно чейнить (цепочкой), чтобы обрабатывать последующие ошибки.
// Важно помнить, что если в цепочке промисов возникает ошибка и она не обрабатывается,
// то выполнение цепочки прерывается и управление передается в ближайший блок catch().
...
//7. микро и макро таски - что выполниться в первую очередь?
// В JavaScript есть две очереди задач: очередь макрозадач и очередь микрозадач.
...
//8. задачи стэк вызова: очередь и инвент луп
console.log(1); 
